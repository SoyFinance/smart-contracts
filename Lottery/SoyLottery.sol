...
// File: contracts/interfaces/ISoyLottery.sol
 ...
 */
contract SoyLottery is ReentrancyGuard, ISoyLottery, Ownable {
    using SafeERC20 for IERC20;
    uint256 constant MAX_BRACKETS = 3;  // maximum winning combination (i.e. first 1, first 2, first 3, ...). (Default: 6)
    ...
    uint256 public maxPriceTicketInSoy = 50 ether;
    uint256 public minPriceTicketInSoy = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSoy;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] soyPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSoy;
        uint32 finalNumber;
    }
     ...
     * @param _soyTokenAddress: address of the SOY token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _soyTokenAddress, address _randomGeneratorAddress) {
        soyToken = IERC20(_soyTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of SOY to this contract
        uint256 amountSoyToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSoy,
            _ticketNumbers.length
        );

        // Transfer soy tokens to this contract
        soyToken.safeTransferFrom(address(msg.sender), address(this), amountSoyToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSoy += amountSoyToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSoyToTransfer
        uint256 rewardInSoyToTransfer;

        for (uint256 i = 0; i < _ticketIds.length; i++) {
            require(_brackets[i] < MAX_BRACKETS, "Bracket out of range"); // Must be between 0 and 5
            ...
            rewardInSoyToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        soyToken.safeTransfer(msg.sender, rewardInSoyToTransfer);

        emit TicketsClaim(msg.sender, rewardInSoyToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSoy) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in SOY for each bracket by starting from the highest one
        for (uint32 i = 0; i < MAX_BRACKETS; i++) {
            uint32 j = uint32(MAX_BRACKETS-1) - i;
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].soyPerBracket[j] =
                ...
                // A. No SOY to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].soyPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInSoy - amountToShareToWinners);

        // Transfer SOY to treasury address
        soyToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        soyToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSoy += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSoy >= minPriceTicketInSoy) && (_priceTicketInSoy <= maxPriceTicketInSoy),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSoy: _priceTicketInSoy,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            soyPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInSoy: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(soyToken), "Cannot be SOY token");
    ...
    /**
     * @notice Set SOY price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSoy: minimum price of a ticket in SOY
     * @param _maxPriceTicketInSoy: maximum price of a ticket in SOY
     */
    function setMinAndMaxTicketPriceInSoy(uint256 _minPriceTicketInSoy, uint256 _maxPriceTicketInSoy)
        external
        onlyOwner
    {
        require(_minPriceTicketInSoy <= _maxPriceTicketInSoy, "minPrice must be < maxPrice");
     ...
     * @param _priceTicket price of a ticket (in SOY)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].soyPerBracket[_bracket];
        } else {
            return 0;
        }
    }
