...
View [SoyLottery.sol on Callisto Testnet](https://testnet-explorer.callisto.network/address/0xefBf55Af146093738982Fe25e69fE966F26670de/contracts).


## Audits

The Soy Lottery V2 has been audited twice so far. View the results below:

[Peckshield's Lottery V2 Audit](https://github.com/peckshield/publications/blob/master/audit\_reports/PeckShield-Audit-Report-SoyLottery-v1.0.pdf)

[Slowmist's Lottery V2 Audit](https://github.com/slowmist/Knowledge-Base/blob/master/open-report/Smart%20Contract%20Security%20Audit%20Report%20-%20Soy%20Lottery.pdf)
...
| `priceTicketInSoy`        | uint256     | The price of a ticket in SOY (approximately $5 USD).                                                                                                                                                                |
...
| `soyPerBracket`           | uint256\[6] | The amount of SOY to distribute to winners of each bracket.                                                                                                                                                         |
...
| `amountCollectedInSoy`    | uint256     | The amount of SOY collected through ticket sales for the Lottery round.                                                                                                                                             |
| `finalNumber`              | uint32      | The final number determined by `randomResult` obtained from the number generator contract ([RandomNumberGenerator.sol](https://testnet-explorer.callisto.network/address/0x09D8B4A17edd82CA0681409329A31DC04B74bae2/contracts)) |
...
Calculates rewards for a ticket after draw given the `lotteryId`, `ticketId`, and `bracket`. Filling and querying will provide a link to detailed price information on [testnet explorer](https://testnet-explorer.callisto.network/address/0xefBf55Af146093738982Fe25e69fE966F26670de/read-contract).
...
Filling and querying will provide a link to detailed price information on Callisto Explorer.

| Name              | Type    | Description                    |
| ----------------- | ------- | ------------------------------ |
| `discountDivisor` | uint256 | The divisor for the discount.  |
| `priceTickets`    | uint256 | The price of a ticket in SOY. |
...
| ----------- | ------- | --------------------------------- |
| `lotteryId` | uint256 | The id of the Lottery.            |
| `amount`    | uint256 | Amount, in SOY token, to inject. |
...
| `priceTicketInSoy` | uint256     | Price of a ticket in SOY.                                 |
...
Allows admin to recover incorrect tokens sent to address mistakenly. Cannot be SOY tokens.
...
### setMinAndMaxTicketPriceInSoy

```
function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner;
```

Allows admin to set upper and lower limit of ticket price in SOY value. Minimum price must be lower than maximum price.

| Name                   | Type    | Description                |
| ---------------------- | ------- | -------------------------- |
| `minPriceTicketInSoy` | uint256 | The minimum price in SOY. |
| `maxPriceTicketInSoy` | uint256 | The maximum price in SOY. |
